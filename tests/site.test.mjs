...
const defaults = {
  collection: "server", downloader: "curl", release: "v0.5.3", user: "", yes: false,
...
test("builds the minimal pinned command", () => {
  assert.equal(buildCommand(defaults), "curl -fsSL https://raw.githubusercontent.com/EckPhi/mistborn-bootstrap/v0.5.3/install.sh | sudo bash -s -- server");
});
...
test("never applies server settings to the shell collection", () => {
  ...
  assert.equal(command, "curl -fsSL https://raw.githubusercontent.com/EckPhi/mistborn-bootstrap/v0.5.3/install.sh | sudo bash -s -- shell");
});
