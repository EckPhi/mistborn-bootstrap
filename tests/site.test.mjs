...
const defaults = {
  collection: "server", downloader: "curl", release: "v0.4.0", user: "", yes: false,
...
test("builds the minimal pinned command", () => {
  assert.equal(buildCommand(defaults), "curl -fsSL https://raw.githubusercontent.com/EckPhi/mistborn-bootstrap/v0.4.0/dist/server.sh | sudo bash");
});
...
test("adds supported server configuration with safe quoting", () => {
  ...
  assert.equal(buildCommand(config), "curl -fsSL https://raw.githubusercontent.com/EckPhi/mistborn-bootstrap/v0.4.0/dist/server.sh | sudo env MISTBORN_TAILSCALE_SSH=1 MISTBORN_TAILSCALE_AUTO_UPDATE=1 MISTBORN_HARDEN=1 MISTBORN_SSH_PORT=2222 MISTBORN_TAILSCALE_ONLY=1 MISTBORN_ALLOWED_TCP_PORTS='80 443' bash -s -- --yes --user phil");
...
test("never applies server settings to the shell collection", () => {
  ...
  assert.equal(command, "curl -fsSL https://raw.githubusercontent.com/EckPhi/mistborn-bootstrap/v0.4.0/dist/shell.sh | sudo bash");
});
